...
const AjouterProduct = () => {
  ...
  const [categories, setCategories] = useState<{ id: string, name: string }[]>([]); // State to hold categories
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        ...
        setCategories(response.data.$values); // Assuming the response is an array of categories
  ...
  const handleValidation = () => {
    ...
    if (!image) formErrors.picture = "Please upload a product picture.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (handleValidation()) {
      ...
      console.log("Logging all values in formData:");
      for (let [key, value] of formData.entries()) {
        console.log(`${key}:`, value);
      }


      await axios.post('https://localhost:7223/api/Product', formData, {
        headers: {
